...
func main() {
	...
	// 🚧 Drop unique constraint on refresh_tokens.user_id — allows multiple concurrent sessions
	// (login from multiple tabs/devices without invalidating each other).
	// Safe to run repeatedly — idempotent (IF EXISTS / DROP IF EXISTS).
	log.Println("Running refresh_tokens migration (drop user_id unique constraint)...")

	// Create a plain (non-unique) index first so queries don't suffer.
	config.DB.Exec(`CREATE INDEX IF NOT EXISTS idx_refresh_tokens_user_id ON refresh_tokens (user_id)`)
	// Drop named unique constraints that GORM or manual migration may have created.
	config.DB.Exec(`ALTER TABLE refresh_tokens DROP CONSTRAINT IF EXISTS refresh_tokens_user_id_key`)
	config.DB.Exec(`ALTER TABLE refresh_tokens DROP CONSTRAINT IF EXISTS uni_refresh_tokens_user_id`)
	config.DB.Exec(`ALTER TABLE refresh_tokens DROP CONSTRAINT IF EXISTS uq_refresh_tokens_user_id`)
	// Drop the unique index that GORM creates from `uniqueIndex` tag.
	// The index name auto-generated by GORM is: uidx_refresh_tokens_user_id
	config.DB.Exec(`DROP INDEX IF EXISTS uidx_refresh_tokens_user_id`)
	config.DB.Exec(`DROP INDEX IF EXISTS idx_refresh_tokens_user_id_unique`)
